...
def getRhat(x0,x1):
    ...
    Rhat = [np.empty([2,2]),np.empty([2,2])]
    for i in range(N0):
        Rhat[0] = Rhat[0] + np.outer(x0[:,i]-mu[0],x0[:,i]+mu[0])
    
    for i in range(N1):
        Rhat[1] = Rhat[1] + np.outer(x1[:,i]-mu[1],x1[:,i]+mu[1])

    return Rhat[0]/(N0-1),Rhat[1]/(N1-1)

def calcDelQDA(data,Rhat,mu,N0,N):
    return (np.log(N0)-np.log(N))-0.5*np.log(np.linalg.norm(Rhat))-0.5*np.dot(np.dot((data-mu).T,np.linalg.inv(Rhat)),data-mu)
...
def getDel(x0,x1,Rhat):
    ...
    for i in range(N0):
        del_l = np.c_[del_l,np.array([
            calcDelQDA(x0[:,i],Rhat[0],mu[0],N0,N), 
            calcDelQDA(x0[:,i],Rhat[1],mu[1],N0,N)]).T ]

    for i in range(N1):
        del_l = np.c_[del_l,np.array([
            calcDelQDA(x1[:,i],Rhat[0],mu[0],N0,N), 
            calcDelQDA(x1[:,i],Rhat[1],mu[1],N0,N)]).T ]
...
for x in xp1:
    for y in yp1:
        del_l =  np.array(
                [calcDelQDA([x,y],Rhat[0],mu[0],N0,N),
                 calcDelQDA([x,y],Rhat[1],mu[1],N1,N)])
