...
%  └────────────────────────┘
    ...
    \DeclareDocumentCommand{\overview}{} {
        ...
        This document is not intended to be a comprehensive teaching document to describe each binary classifier, but rather aims to analyze the differences between them as discussed in \reff{sec:discussion}.
    } 
    ...
    \DeclareDocumentCommand{\lqAnalysis}{} {
        ...
        The plotted data for Problem 7 can be seen in \reff{fig:qda}. The co-variance matrix can be seen in the second code block below, and it is referenced to as \texttt{Rhat} in the code. The decision boundaries between the LDA and QDA are significantly different; the LDA has a linear shape almost exactly the same as the Linear Regression, whereas the QDA has a steep curve towards the class1 data as seen in \reff{fig:qda}. This curve can allow the classifier to be more sensitive to nonlinearities.
    ...
    \DeclareDocumentCommand{\kNearest}{} {
        ...
        As seen in the following code, the $k$ value can be easily manipulated to give the corresponding results seen in \reff{fig:k1} through \reff{fig:k15}. 
        ...
        The classification regions can be seen in \reff{fig:k1} through \reff{fig:k15}.
        ...
        \newFigure{./media/1-nearest.pdf}{k=1 Nearest Neighbor}{0.45}{fig:k1}
        \newFigure{./media/5-nearest.pdf}{k=5 Nearest Neighbor}{0.45}{fig:k5}
    ...
    \DeclareDocumentCommand{\naiveBayes}{} {
        \section{Naive Bayes Classifier}
            \subsection{Problem 14:} The true density is a Gaussian mixture,$f_X(x) = p_0 N(x, \mu_0,\sigma_0^2) + p_1 N(x, \mu_1 , \sigma_1^2)$. Using this equation, with given values of $\mu$ and $sigma$, make a plot of the true density, the sample empirical distribution, the kernel functions, and the estimated density. Try different values of $\lambda$.

            As seen in \reff{fig:parzen}, the Parzen distribution was calculated using an empirical distribution and adding the corresponding Gaussian distributions. The $\lambda$ of $\lambda=0.8$ was chosen for the best results. If $\lambda$ is too small, the data begins to look corrupt, and if it was much larger than $\lambda=0.8$, it became too peaked in the middle regions.
    ...
    \DeclareDocumentCommand{\discussion}{} {
        ...
        The highest performing data classifier was the Bayes Optimal classifier. In this case it was because we had the exact model of the data, but in real world examples, it is not feasible to have such a model.
        ...
        The 15-nearest neighbor performed as well as the Bayes Optimal classifier, but it took considerably longer. There exist ways to optimize the code, because I was comparing the distance of every point in the data set to the test point, which required a lot of wasted time. Regardless, one of the major drawbacks of the nearest neighbor, is that it takes a considerable amount of time to run through the algorithm.
